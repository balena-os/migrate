...
use crate::{
    ...
    linux_common::{call_cmd, restore_backups, MigrateInfo,disk_info::path_info::PathInfo, MKTEMP_CMD, CHMOD_CMD},
...
impl BootManager for UBootManager {
    ...
    fn setup(&self, mig_info: &mut MigrateInfo) -> Result<(), MigError> {
        ...
        let drive_num = {
            let dev_name = &mig_info.get_boot_pi().device;
        ...
        let dtb_path = path_append(&mig_info.get_boot_pi().path, MIG_DTB_NAME);
        ...
        let bootmgr_path = if let Some(bootmgr_path) = mig_info.get_bootmgr_pi() {
            bootmgr_path
        } else {
            mig_info.get_root_pi()
        };
        ...
        if file_exists(&uenv_path) {
            ...
            if !is_balena_file(&uenv_path)? {
                ...
                mig_info
                    .boot_cfg_bckup
                    .push((String::from(&*uenv_path.to_string_lossy()), backup_uenv));
            }
        }
        ...
        uenv_text = uenv_text.replace(
            "__ROOT_DEV__",
            &mig_info.get_root_pi().device.to_string_lossy(),
        );
    ...
    fn set_bootmgr_path(&self,config: &Config, mig_info: &mut MigrateInfo) -> Result<bool, MigError> {
        ...
        let (root_dev, root_part) = mig_info.get_lsblk_info().get_path_info(ROOT_PATH)?;
                ...
                if let Some(ref fstype) = partition.fstype {
                    if fstype == "vfat" || fstype.starts_with("ext") {
                        ...
                        let mountpoint = match partition.mountpoint {
                            ...
                            None => {
                                ...
                                if let None = tmp_mountpoint {
                                    let cmd_res = call_cmd(MKTEMP_CMD, &["-d", "-p", &mig_info.get_work_path().to_string_lossy()], true)?;
                            ...
                            || file_exists(path_append(mountpoint, UBOOT_FILE_NAME))
                        {
                            ...
                            mig_info.set_bootmgr_pi(Some(PathInfo::from_mounted(
