...
impl<'a> WifiConfig {
    ...
    fn parse_conmgr_file(file_path: &Path) -> Result<Option<WifiConfig>,MigError> {
        ...
        let file = File::open(file_path).context(MigErrCtx::from_remark(MigErrorKind::Upstream, &format!("failed to open file {}", file_path.display())))?;        
        ...
        for line in BufReader::new(file).lines() { 
            match line {
                ...
                Err(why) => {
                    return Err(MigError::from(why.context(MigErrCtx::from_remark(MigErrorKind::Upstream, &format!("unexpected read error from {}", file_path.display())))));
                },
            }
        }
    ...
    fn from_connman(wifis: &mut Vec<WifiConfig>, ssid_filter: &Vec<String>) -> Result<(),MigError> {
        if dir_exists(CONNMGR_CONFIG_DIR)? {
            let paths = read_dir(CONNMGR_CONFIG_DIR)
                .context(MigErrCtx::from_remark(MigErrorKind::Upstream, &format!("Failed to list directory '{}'", CONNMGR_CONFIG_DIR)))?;
            ...
            for path in paths {
                if let Ok(path) = path {                    
                    ...
                    if let Some(dir_name) = dir_path.file_name() {
                        if dir_name.to_string_lossy().starts_with("wifi_") && dir_path.metadata().unwrap().is_dir() {
                            debug!("examining connmgr path '{}'", dir_path.display());
                            let settings_path = dir_path.join("settings");
                            if settings_path.exists() {
                                ...
                                if let Some(wifi) = WifiConfig::parse_conmgr_file(&settings_path)? {
                                    ...
                                    if ! valid {
                                        if let Some(_pos) = ssid_filter.iter().position(|r| r.as_str() == wifi.ssid) {
                                            valid = true;
                                        }
                                    }
                                    if valid {
                                        if let Some(_pos) = wifis.iter().position(|r| r.ssid == wifi.ssid) {
                            ...
                            debug!("no match on '{}' starts_with(wifi_): {} is_dir: {}", dir_path.display(), dir_name.to_string_lossy().starts_with("wifi_"), dir_path.metadata().unwrap().is_dir());
                        }
                    } else {
                        warn!("Not processing invalid path '{}'", path.path().display());    
                    }
                } else {
                    return Err(MigError::from_remark(MigErrorKind::Upstream, &format!("Error reading entry from directory '{}'", CONNMGR_CONFIG_DIR)));
                }                
            }
        } else {
            debug!("WifiConfig::from_connman: directory not found: '{}'", CONNMGR_CONFIG_DIR);
        }
    ...
    fn from_wpa(wifis: &mut Vec<WifiConfig>, ssid_filter: &Vec<String>) -> Result<(),MigError> {
        ...
        if file_exists(WPA_CONFIG_FILE) {
            ...
            let file = File::open(WPA_CONFIG_FILE).context(MigErrCtx::from_remark(MigErrorKind::Upstream, &format!("failed to open file {}", WPA_CONFIG_FILE)))?;
            ...
            for line in BufReader::new(file).lines() {
                ...
                match line {
                        ...
                        match state {
                            ...
                            WpaState::Network => {
                                ...
                                if let Some(captures) = net_param1_re.captures(&line) {
                                    let param = captures.get(1).unwrap().as_str();
                                    let value = captures.get(2).unwrap().as_str();
                                    debug!("in state {:?} got param: '{}', value: '{}'", state, param, value);
                                    match param {
                                        "ssid" => {                                            
                    ...
                    Err(why) => {
                        return Err(MigError::from(why.context(MigErrCtx::from_remark(MigErrorKind::Upstream, &format!("unexpected read error from {}", WPA_CONFIG_FILE)))));
                    },                
                }                
            }
        } else {
            debug!("WifiConfig::from_wpa: file not found: '{}'", WPA_CONFIG_FILE);
        }

        Ok(())
    }

    pub(crate) fn create_nwmgr_file<P: AsRef<Path>>(&self, base_path: P, last_index: u64) -> Result<u64,MigError> {
        let mut index = last_index + 1;
        let mut file_ok = false;
        let base_path = base_path.as_ref();
        let mut path = base_path.join(&format!("resin-wifi-{}", index));
        ...
        while ! file_ok {
            if file_exists(&path) {
                if is_balena_file(&path)? {
                    ...
                    index += 1;
                    path = base_path.join(&format!("resin-wifi-{}", index));
                }
            } else {
                file_ok = true;                    
            }
        ...
        let mut nwmgr_file = File::create(&path).context(MigErrCtx::from_remark(MigErrorKind::Upstream, &format!("Failed to create file in '{}'", path.display())))?;
        ...
        nwmgr_file.write_all(content.as_bytes()).context(MigErrCtx::from_remark(MigErrorKind::Upstream, &format!("failed to write new '{:?}'", path.display())))?;    
        Ok(index)
    }
