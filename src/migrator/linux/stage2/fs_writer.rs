...
use crate::{
    common::{stage2_config::Stage2Config, MigErrCtx, MigError, MigErrorKind},
    linux::{
...
pub(crate) fn partition(
    ...
    let mut sfdisk_cmd = Command::new(sfdisk_path)
        ...
        .context(MigErrCtx::from_remark(
            ...
            &format!("Failed to start command : '{}'", sfdisk_path),
        ))?;

    {
        if let Some(ref mut stdin) = sfdisk_cmd.stdin {
            let _res = stdin
                .write("label: dos".as_bytes())
                .context(MigErrCtx::from_remark(
                    MigErrorKind::Upstream,
            ...
            for curr_part in partitions {
                ...
                let _res = stdin
                    .write(
                        &format!(
                            "start={},size={},bootable={},type={:x}",
                            curr_part.start_lba, curr_part.num_sectors, bootable, curr_part.ptype
                        )
                        .as_bytes(),
                    )
                    .context(MigErrCtx::from_remark(
    ...
    let cmd_res = sfdisk_cmd
        .wait_with_output()
        .context(MigErrCtx::from_remark(
            MigErrorKind::Upstream,
            &format!("failure waiting for sfdisk to terminate"),
        ))?;
