...
impl Logger {
    pub fn initialise(default_log_level: usize) -> Result<(),MigError> {
        ...
        if let Ok(config_path) = env::var("LOG_CONFIG") {
            let config_str = &read_to_string(&config_path).context(MigErrCtx::from_remark(MigErrorKind::Upstream, &format!("{}::from_file: failed to read {}", MODULE, config_path)))?;
            ...
            if yaml_cfg.len() > 1 {
                return Err(MigError::from_remark(MigErrorKind::InvParam,&format!("{}::from_string: invalid number of configs in file: {}, {}", MODULE, config_path, yaml_cfg.len())));
            }
