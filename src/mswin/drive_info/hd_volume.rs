...
use crate::{MigError};
use super::{HarddiskPartitionInfo, DeviceProps};
...
impl<'a> HarddiskVolumeInfo {
    pub fn try_from_device(device: &str) -> Result<Option<HarddiskVolumeInfo>,MigError> {
        ...
        if let Some(cap) = RE_HDVOL.captures(device) {
            Ok(Some(HarddiskVolumeInfo::new(device, cap.get(1).unwrap().as_str().parse::<u64>().unwrap())?))
        } else {
            Ok(None)
        }
    }
 ...
 fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if let Some(hdp) = &self.hd_part {
            if let Some(hdp) = hdp.upgrade() {
                dep_dev = format!("HarddiskPartition({},{})",hdp.as_ref().borrow().get_hd_index(),hdp.as_ref().borrow().get_part_index())
            } else {
                // consider error
