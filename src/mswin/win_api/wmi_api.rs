    ...
    pub fn get_api_from_hcom(h_com_api: HComApi) -> Result<WmiAPI,MigError> {
        ...
        if unsafe {
            CoCreateInstance(
                ...
                warn!("{}::get_api_from_hcom: CoCreateInstance returned os error: {:?} ", MODULE, os_err);
                return Err(
                    MigError::from(
                        os_err.context(
                            MigErrCtx::from_remark(MigErrorKind::WinApi, &format!("{}::get_api_from_hcom: CoCreateInstance failed",MODULE)))));
        }
        ...
        if unsafe {
            (*(p_loc as PMIWbemLocator)).ConnectServer(
                ...
                let os_err = Error::last_os_error();
                warn!("{}::get_api_from_hcom: ConnectServer returned os error: {:?} ", MODULE, os_err);
                return Err(
                    MigError::from(
                        os_err.context(
                            MigErrCtx::from_remark(MigErrorKind::WinApi, &format!("{}::get_api_from_hcom: ConnectServer failed",MODULE)))));                
        }
        ...
        if unsafe { CoSetProxyBlanket(
            ...
            let os_err = Error::last_os_error();
            warn!("{}::get_api_from_hcom: CoSetProxyBlanket returned os error: {:?} ", MODULE, os_err);
            return Err(
                MigError::from(
                    os_err.context(
                        MigErrCtx::from_remark(MigErrorKind::WinApi, &format!("{}::get_api_from_hcom: CoSetProxyBlanket failed",MODULE)))));
        }
