...
fn parse_mem_value(val: Option<&&str> )  -> Result<usize,MigError> {
    ...
    let captures = match RE.captures(val) {
        Some(c) => c,
        None => return Err(MigError::from_code(MigErrorCode::ErrInvParam, &format!("{}::parse_mem_value: failed to parse memory field from '{}' - no match on regex",MODULE, val) ,None)),
    };                
    ...
    let digits = match captures.get(2) {
        ...
        None => {
            match captures.get(4) {
                ...
                None => return Err(MigError::from_code(
                    MigErrorCode::ErrInvParam, 
                    &format!("{}::parse_mem_value: failed to parse memory from '{}' - no digits captured ", MODULE, val) ,None)),
            }
        },
    };
